        ...
        switch ($viewName) {
            ...
            case 'EditAnticipo':
                ...
                if (false === $modelExists) {
					...
					if (empty($model->importe)) {
						...
						if (!empty($model->idalbaran)) {
							$doc = \FacturaScripts\Dinamic\Model\AlbaranCliente::find($model->idalbaran);
							if ($doc) {
							...
							$doc = \FacturaScripts\Dinamic\Model\PedidoCliente::find($model->idpedido);
							if ($doc) {
								$totalDoc = (float) $doc->total;
							...
							$doc = \FacturaScripts\Dinamic\Model\PresupuestoCliente::find($model->idpresupuesto);
							if ($doc) {
								$totalDoc = (float) $doc->total;
							...
							$doc = \FacturaScripts\Dinamic\Model\FacturaCliente::find($model->idfactura);
							if ($doc) {
								$totalDoc = (float) $doc->total;
				...
				if (false === empty($model->idalbaran) && false === $modelExists) {
                    ...
                    $model->fase = "Proyecto";
				} elseif (false === empty($model->codcliente) && false === $modelExists) {
                    $model->fase = "Cliente";
